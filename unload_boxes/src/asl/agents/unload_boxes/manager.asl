...
+!start
	<- 	scenario_unloadBoxes.actions.generic.getTime(StartTime);
		+startTime(StartTime);
.
 ...
 */ 
+stop(Truck) 
	<-	scenario_unloadBoxes.actions.generic.resetProperties(Truck);
		scenario_unloadBoxes.actions.trucker.getNext(Next_trucker);
...
+!check_end(Next_trucker)
	<- 	?startTime(StartTime);
		...
		if(Added == Finished)
		{
			scenario_unloadBoxes.actions.generic.getTime(EndTime);
...
+!quit[source(Truck)]
	<-	scenario_unloadBoxes.actions.generic.resetProperties(Truck);
		scenario_unloadBoxes.actions.trucker.goToWaitingQueue(Truck);
		scenario_unloadBoxes.actions.trucker.getNext(Next_trucker);
		.send(Next_trucker, achieve, start);
		.send(Next_trucker, achieve, unload);
		update_screen;
